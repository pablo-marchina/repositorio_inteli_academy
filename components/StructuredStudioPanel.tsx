...
export function StructuredStudioPanel({ projectId, driveAssets, versions, initialVersionId }: { projectId: string; driveAssets: DriveAsset[]; versions: StructuredVersion[]; initialVersionId?: string | null }) {
  ...
  return <section className={styles.card} style={{ marginBottom: 20 }}>
    ...
    <p>Scene Graph canônico + Figma como fonte visual. Para Reels, o score estrutural não é mais tratado como prova de qualidade: timeline e render real têm gates separados.</p>
    <div style={{ display: "grid", gap: 12 }}>
      ...
      {artifact.renderQa ? <Score title="Crítico visual · render real do Reel" report={artifact.renderQa} /> : null}
    </div>
    ...
    {artifact.videoTimeline ? <div style={{ marginTop: 22, display: "grid", gap: 14 }}>
      <div><h3>Timeline de vídeo editável</h3><p>Footage, áudio, texto e gráficos são tracks independentes. O preview pós-Figma dispara automaticamente o QA de cinco frames do render real.</p></div>
      <StudioVideoPreview payload={selected.payload} timeline={artifact.videoTimeline} driveAssets={driveAssets} figmaLayout={artifact.figmaVideoLayout} projectId={projectId} versionId={selected.id} initialRenderQa={artifact.renderQa} />
      ...
      {!selected.figma_frame_ids.length ? <p style={{ fontSize: 13, opacity: .72 }}>O export nativo e o QA visual final aparecem depois que a versão passa pelo Figma.</p> : null}
      <p style={{ fontSize: 13, opacity: .7 }}>O score de timeline valida 6–12 shots, diversidade, in/out contra duração real, áudio, smart crop e ritmo. O score do render é independente e avalia o que efetivamente aparece no Player.</p>
    </div> : null}
  </section>;
}
