...
async function cacheTemporalReference(references: InstagramReferencePost[], plan: ReelEditingPlan | undefined) {
  if (!plan?.reference) return;
  const target = references.find((reference) => reference.mediaType === "VIDEO" || reference.mediaProductType === "REELS" || reference.mediaProductType === "REEL");
  if (!target) return;
  const visualAnalysis = { ...(target.visualAnalysis ?? {}), temporal: plan.reference };
  target.visualAnalysis = visualAnalysis;
  await createAdminClient().from("instagram_reference_posts").update({ visual_analysis: visualAnalysis }).eq("id", target.id);
}
...
async function historicalInstagramGuidance() {
  ...
  for (const row of rows) {
    ...
    if (row.visual_analysis && typeof row.visual_analysis === "object" && Object.keys(row.visual_analysis as object).length) analyzed.push(JSON.stringify(row.visual_analysis));
  }
...
async function loadArticles(articleIds: string[]): Promise<StudioArticleEvidence[]> {
  ...
  return articleIds.map((id) => {
    ...
    return { id, title: String(row.title), summary: String(row.summary ?? ""), url: String(row.canonical_url), source: String(row.source_name), publishedAt: String(row.published_at) };
  });
}
...
function mergeCurrentFigmaContent(payload: StudioPayload, state: Awaited<ReturnType<typeof getCurrentFigmaSemanticState>>) {
  return {
    ...
    frames: payload.frames.map((frame, index) => {
      ...
      return { ...frame, eyebrow: firstSemanticText(semantic, "eyebrow") ?? frame.eyebrow, title: firstSemanticText(semantic, "headline") ?? frame.title, body: firstSemanticText(semantic, "body") ?? frame.body, stat: firstSemanticText(semantic, "stat") ?? frame.stat, statLabel: firstSemanticText(semantic, "statLabel") ?? frame.statLabel, bullets: bullets.length ? bullets : frame.bullets };
    })
...
function reusableReelPlan(payload: StructuredStudioPayload) {
  ...
  const referenceIsCurrent = !plan.reference || (plan.reference.semanticVersion ?? 0) >= 4;
  ...
  return semanticVersion >= 4 && referenceIsCurrent && everyUsedShotWasAnalyzed && musicIsCurrent ? plan : undefined;
}
...
export async function createStudioProject(rawInput: unknown, userId: string) {
  ...
  const [articles, references, selectedDriveAssets, history] = await Promise.all([
  ...
  const reelPlan = input.contentType === "reel"
    ...
    : undefined;
  const driveAssets = selectedDriveAssets;
  ...
  const admin = createAdminClient();
  const { data: project, error: projectError } = await admin.from("content_projects").insert({ created_by: userId, name: payload.title, content_type: input.contentType, article_ids: input.articleIds, user_context: input.userContext, instagram_reference_media_id: referenceIds[0] ?? null, instagram_reference_media_ids: referenceIds, use_drive: input.useDrive, drive_assets: driveAssets, status: "generated" }).select("id").single();
  if (projectError) throw projectError;
  const { data: version, error: versionError } = await admin.from("content_versions").insert({ project_id: project.id, version_number: 1, payload, change_request: "Versão inicial", status: "generated", created_by: userId }).select("id,version_number").single();
...
export async function createStudioRevision(projectId: string, baseVersionId: string, changeRequest: string, userId: string) {
  ...
  const baseStructured = base.payload as StructuredStudioPayload;
  ...
  if (baseFigmaFrameIds.length) {
    try { current = mergeCurrentFigmaContent(parsedCurrent, await getCurrentFigmaSemanticState(baseFigmaFrameIds)); } catch { current = parsedCurrent; }
  }
  const articleIds = Array.isArray(project.article_ids) ? project.article_ids.map(String) : [];
  const driveAssets = Array.isArray(project.drive_assets) ? (project.drive_assets as DriveAsset[]).filter((asset) => !asset.mimeType.startsWith("audio/")) : [];
...
Revisão: ${changeRequest}` })
  ...
  if (error) throw error;
  await admin.from("content_projects").update({ name: compiled.title, status: "generated", drive_assets: driveAssets, last_error: null }).eq("id", projectId);
...
export async function queueStudioVersionForFigma(projectId: string, versionId: string) {
  ...
  const [{ data: project, error: projectError }, { data: version, error: versionError }] = await Promise.all([
    admin.from("content_projects").select("id,name,content_type,drive_assets,instagram_reference_media_id,instagram_reference_media_ids").eq("id", projectId).single(),
...
export async function nextFigmaJob() {
  const { data, error } = await createAdminClient().from("figma_jobs").select("id,project_id,version_id,payload,created_at").eq("status", "queued").order("created_at", { ascending: true }).limit(1).maybeSingle();
  if (error) throw error;
  return data ?? null;
}
...
export async function completeFigmaJob(jobId: string, frameIds: string[], templateNodeIds: string[] = []) {
  ...
  try {
    const semanticState = await getCurrentFigmaSemanticState(frameIds);
    versionPayload = attachFigmaVideoLayout(versionPayload, semanticState);
  } catch {
    // A importação editável continua válida, mas o preview de Reel fica explicitamente marcado como pré-Figma até o layout poder ser lido.
  }
  if (jobPayload.payload && templateNodeIds.length === frameIds.length) {
    try {
      const [outputRenderUrls, sourceRenderUrls] = await Promise.all([getCurrentFigmaRenderUrls(frameIds, "png"), getCurrentFigmaRenderUrls(templateNodeIds, "png")]);
      const visualBrandReview = await reviewFigmaBrandFidelity({ payload: jobPayload.payload, outputRenderUrls, sourceRenderUrls });
      if (visualBrandReview && versionPayload.artifact) versionPayload = { ...versionPayload, artifact: { ...versionPayload.artifact, visualBrandReview } };
    } catch {
      // O crítico de Figma é um gate adicional; uma falha transitória não destrói a importação editável.
    }
  }
