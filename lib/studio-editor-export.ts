  ...
  const tracks = timeline.tracks.map((track) => {
    ...
    if (track.startFrame > 0) {
      children.push({
        OTIO_SCHEMA: "Gap.1",
        name: "Pre-roll",
        source_range: timeRange(0, track.startFrame, timeline.fps),
        metadata: {}
      });
    }
    children.push({
      ...
      media_reference: track.assetId ? {
        ...
        metadata: {
          academy: {
            ...
            role: track.role,
            text: track.text ?? null,
            note: "Use o manifest da Academy para reconstruir esta camada como texto/gráfico nativo no editor."
          }
        }
      }
    });
...
function manifestTrack(track: StudioVideoTrack, assets: DriveAsset[], assetUrl: (assetId: string) => string) {
  const asset = track.assetId ? assets.find((item) => item.id === track.assetId) : null;
  return {
    ...track,
    startSeconds: track.startFrame,
    source: asset ? {
      id: asset.id,
      name: asset.name,
...
export function serializeEditorManifest(input: {
  ...
  return {
    ...
    remotion: artifact.videoTimeline ? {
      ...
      timeline: {
        ...artifact.videoTimeline,
        tracks: artifact.videoTimeline.tracks.map((track) => manifestTrack(track, input.driveAssets, input.assetUrl))
      }
    } : null,
    payload: {
      title: input.payload.title,
      caption: input.payload.caption,
      frames: input.payload.frames,
      factualClaims: input.payload.factualClaims
    }
  };
}
