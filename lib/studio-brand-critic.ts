...
import { effectiveBrandContext, effectivePostArchetype, requiresPartnerBrand } from "@/lib/studio-post-archetype";
...
function brandInstruction(payload: StudioPayload) {
  ...
  return `Arquétipo editorial: ${archetype}. Marca principal: ${brand.primaryBrandName}. ${requiresPartnerBrand(payload)
    ? `Há parceiro${brand.partnerName ? ` (${brand.partnerName})` : ""}: exija uma partnerLogo real e separada da primaryLogo. Não aceite texto improvisado, recorte incidental de foto/vídeo ou símbolo redesenhado como logo.`
