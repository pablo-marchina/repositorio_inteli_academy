...
function googleConfig() {
  const config = env();
  if (!config.GOOGLE_CLIENT_ID || !config.GOOGLE_CLIENT_SECRET) throw new Error("GOOGLE_CLIENT_ID e GOOGLE_CLIENT_SECRET são necessários para conectar o Drive.");
  return config;
}
...
export async function exchangeGoogleDriveCode(code: string) {
  const config = googleConfig();
  const body = new URLSearchParams({ code, client_id: config.GOOGLE_CLIENT_ID!, client_secret: config.GOOGLE_CLIENT_SECRET!, redirect_uri: `${config.NEXT_PUBLIC_APP_URL}/api/drive/callback`, grant_type: "authorization_code" });
  ...
  return { accessToken: tokens.access_token, refreshToken: tokens.refresh_token ?? null, expiresAt: tokens.expires_in ? new Date(Date.now() + tokens.expires_in * 1000).toISOString() : null, email: profile.email ?? null };
}

export async function saveDriveConnection(input: { accessToken: string; refreshToken: string | null; expiresAt: string | null; email: string | null; connectedBy: string }) {
  const admin = createAdminClient();
  const existing = await admin.from("drive_connections").select("refresh_token_encrypted").eq("id", true).maybeSingle();
  const refreshTokenEncrypted = input.refreshToken ? encryptSecret(input.refreshToken) : existing.data?.refresh_token_encrypted ?? null;
  const { error } = await admin.from("drive_connections").upsert({ id: true, google_email: input.email, root_folder_id: env().GOOGLE_DRIVE_ROOT_FOLDER_ID, access_token_encrypted: encryptSecret(input.accessToken), refresh_token_encrypted: refreshTokenEncrypted, token_expires_at: input.expiresAt, is_active: true, connected_by: input.connectedBy });
  if (error) throw error;
}

async function activeConnection(): Promise<DriveConnectionRow> {
  const { data, error } = await createAdminClient().from("drive_connections").select("root_folder_id,access_token_encrypted,refresh_token_encrypted,token_expires_at,google_email").eq("id", true).eq("is_active", true).maybeSingle();
  if (error) throw error;
...
async function refreshedAccessToken(connection: DriveConnectionRow) {
  ...
  const config = googleConfig();
  const body = new URLSearchParams({ client_id: config.GOOGLE_CLIENT_ID!, client_secret: config.GOOGLE_CLIENT_SECRET!, refresh_token: decryptSecret(connection.refresh_token_encrypted), grant_type: "refresh_token" });
  const response = await fetch("https://oauth2.googleapis.com/token", { method: "POST", headers: { "content-type": "application/x-www-form-urlencoded" }, body, cache: "no-store" });
  if (!response.ok) throw new Error(`Falha ao renovar Google Drive: ${await response.text()}`);
  const token = (await response.json()) as GoogleTokenResponse;
  const expiresAt = token.expires_in ? new Date(Date.now() + token.expires_in * 1000).toISOString() : null;
  await createAdminClient().from("drive_connections").update({ access_token_encrypted: encryptSecret(token.access_token), token_expires_at: expiresAt }).eq("id", true);
...
export async function listDriveMedia(): Promise<DriveAsset[]> {
  ...
  while (queue.length && visited.size < 500) {
    ...
    let pageToken = "";
    do {
      const payload = await driveFetch<{ files?: DriveFile[]; nextPageToken?: string }>("files", { q: `'${current.id.replace(/'/g, "\\'")}' in parents and trashed = false`, fields: `nextPageToken,files(${MEDIA_FIELDS})`, pageSize: "1000", orderBy: "folder,name", ...(pageToken ? { pageToken } : {}) });
      for (const file of payload.files ?? []) {
        if (file.mimeType === FOLDER_MIME) queue.push({ id: file.id, path: [...current.path, file.name] });
        else if (MEDIA_PREFIXES.some((prefix) => file.mimeType.startsWith(prefix))) assets.push(asAsset(file, current.path));
      }
...
export async function getDriveAsset(fileId: string): Promise<DriveAsset> {
  const file = await driveFetch<DriveFile>(`files/${encodeURIComponent(fileId)}`, { fields: MEDIA_FIELDS });
  if (!MEDIA_PREFIXES.some((prefix) => file.mimeType.startsWith(prefix))) throw new Error("O arquivo selecionado não é uma imagem, vídeo ou áudio compatível.");
  return asAsset(file, []);
}
...
export async function driveConnectionSummary() {
  const { data } = await createAdminClient().from("drive_connections").select("google_email,root_folder_id,is_active,token_expires_at").eq("id", true).maybeSingle();
  return data ?? null;
}
