...
function clipForTrack(track: StudioVideoTrack, timeline: StudioVideoTimeline, mediaUrl: string | null) {
  ...
  return {
    ...
    media_reference: mediaUrl
      ...
      : missingReference({ generatedLayer: true, role: track.role, figmaNodeId: track.figmaNodeId ?? null, text: track.text ?? null })
  };
}
...
export function resolveManifest(input: {
  ...
  return {
    ...
    nativeEditIntent: {
      ...
      music: "Music remains an independent audio track and retains the AI selection rationale/beat metadata.",
